function initializeMultiSelectDropdown(params) {
  let config = {
    search: true,
    hideX: false,
    placeholder: 'Select...',
  ...
  document.querySelectorAll('select[multiple]').forEach((multiSelect) => {
    ...
    div.id = multiSelect.id + '-dropdown';
    ...
    multiSelect.loadOptions = () => {
      ...
      if (config.selectAll || multiSelect.attributes['select-all']?.value === 'true') {
        ...
        optionElementAll.addEventListener('click', () => {
          ...
          optionElementAll.querySelector('input').checked = ch;
      ...
      Array.from(multiSelect.options).map((option) => {
        let optionElement = newElement('div', { class: option.selected ? 'checked' : '', target: option });
...
function getSelectValuesBySelectId(id) {
...
function getSelectValues(selectElement) {
  return [...selectElement.options].filter(option => option.selected)
    .map(option => option.value || option.text);
}

function resetSelectedValues(selectElement) {
  [...selectElement.options].forEach(option => {
    option.selected = false;
    option.optionElement.classList.remove('checked');
    option.optionElement.querySelector('input').checked = false;
    option.optionElement.target.selected = false;
  });
}

function setSelectedValues(selectElement, optionValues) {
  [...selectElement.options].forEach(option => {
    if (optionValues.includes(option.value)) {
      option.selected = true;
      option.optionElement.classList.add('checked');
      option.optionElement.querySelector('input').checked = true;
      option.optionElement.target.selected = true;
    } else {
      option.selected = false;
      option.optionElement.classList.remove('checked');
      option.optionElement.querySelector('input').checked = false;
